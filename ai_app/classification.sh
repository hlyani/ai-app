...
tar -zxvf $HOME/classification.tgz -C $HOME/AscendProjects/
# cd $HOME/AscendProjects/classification
# mkdir -p build/intermediates/host
# cd build/intermediates/host
# cmake ../../../src -DCMAKE_CXX_COMPILER=g++ -DCMAKE_SKIP_RPATH=TRUE
# make
echo "AI calculation..."
cd $HOME/AscendProjects/classification/out
chmod +x main
./main $HOME/data

retry_count=0
for ((retry_count = 1; retry_count <= 3; retry_count++)); do
    echo sshpass -p ${OUT_PASSWORDD} scp -o StrictHostKeyChecking=no -r $HOME/AscendProjects/classification/out/output ${OUT_USER}@${OUT_HOST}:${OUT_PATH}
    sshpass -p ${OUT_PASSWORDD} scp -o StrictHostKeyChecking=no -r $HOME/AscendProjects/classification/out/output ${OUT_USER}@${OUT_HOST}:${OUT_PATH}
    if [ $? -eq 0 ]; then
        break
    fi
done
...
if [ $retry_count -le 3 ]; then
    echo "Return result to "${OUT_USER}@${OUT_HOST}:${OUT_PATH}/classification
else
    echo "Return result failed."
fi

echo "Clean and reset Atlas environment..."
rm -rf $HOME/classification.*
rm -rf $HOME/data/*
